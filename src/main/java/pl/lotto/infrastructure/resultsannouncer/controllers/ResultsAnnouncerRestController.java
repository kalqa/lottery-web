...
public class ResultsAnnouncerRestController {
    ...
    @GetMapping(value = "/api/v1/results/{id}")
    public ResponseEntity<AnnouncerResponseDto> getResultsForUuid(@PathVariable("id") String ticketId) {
        UUID uuid = UUID.fromString(ticketId);
        AnnouncerResponseDto resultsForId = resultsAnnouncerFacade.getResultsForId(uuid);
